...
func (s *Store) ClaimProgress(ctx context.Context, lease time.Duration) (planning.Message, bool, error) {
	...
	err := s.db.QueryRowContext(ctx, `WITH candidate AS (SELECT event_id FROM progress_outbox WHERE published_at IS NULL AND (lease_until IS NULL OR lease_until<now()) ORDER BY request_id,revision FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE progress_outbox SET lease_token=$1,lease_until=now()+$2::interval FROM candidate WHERE progress_outbox.event_id=candidate.event_id RETURNING progress_outbox.event_id,request_id,payload`, m.Token, lease.String()).Scan(&m.ID, &m.RequestID, &m.Payload)
	if errors.Is(err, sql.ErrNoRows) {
		return m, false, nil
	}
...
func (s *Store) AckProgress(ctx context.Context, m planning.Message) error {
	_, err := s.db.ExecContext(ctx, `UPDATE progress_outbox SET published_at=now(),lease_token=NULL,lease_until=NULL WHERE event_id=$1 AND lease_token=$2 AND lease_until>now()`, m.ID, m.Token)
	return err
}
