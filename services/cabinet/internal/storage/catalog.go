...
func (s *Store) ReplaceCatalog(ctx context.Context, snapshot catalog.Snapshot) error {
	...
	for start := 0; start < len(snapshot.Cities); start += 500 {
		...
		for _, c := range snapshot.Cities[start:min(start+500, len(snapshot.Cities))] {
			...
			rows = append(rows, goqu.Record{"source": snapshot.Source, "source_id": c.SourceID, "name": c.Name, "name_ru": c.NameRu, "iata_code": c.IATACode, "region_name": c.RegionName, "country_code": c.CountryCode, "region_code": c.RegionCode, "timezone": c.Timezone, "aliases": string(aliases), "latitude": c.Latitude, "longitude": c.Longitude, "population": c.Population, "active": true})
		}
		update := goqu.Record{}
		for _, col := range []string{"name", "name_ru", "iata_code", "region_name", "country_code", "region_code", "timezone", "aliases", "latitude", "longitude", "population", "active"} {
