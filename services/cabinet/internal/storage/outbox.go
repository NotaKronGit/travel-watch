...
func (s *Store) ClaimOutbox(ctx context.Context, lease time.Duration) (OutboxMessage, bool, error) {
 ...
 UPDATE outbox_events e SET lease_until=now()+($1*interval '1 millisecond'),lease_token=gen_random_uuid(),
 attempts=CASE WHEN attempts<2147483647 THEN attempts+1 ELSE attempts END
 FROM candidate c
 WHERE e.id=c.id
 RETURNING e.id,e.request_id,e.payload,e.lease_token,e.attempts,e.event_type`, lease.Milliseconds()).Scan(&m.ID, &m.RequestID, &m.Payload, &m.LeaseToken, &m.Attempts, &m.EventType)
