...
test('timetable tab preserves station time, shows the transfer window and stops polling after completion',async({page})=>{
 ...
 expect(copied).toContain('Схема 1 · Подвоз 2');
 expect(copied).toContain('Переезд Тестовый вокзал → Тестовый аэропорт: на переезд 8 ч 1 мин');
 expect(copied).toContain('Отправление: 2027-01-10 23:00 +03:00');
 const downloadPromise=page.waitForEvent('download');
 await page.getByRole('button',{name:'Скачать стыковки TXT',exact:true}).click();
 const download=await downloadPromise;
 expect(download.suggestedFilename()).toBe('travel-watch-connections.txt');
