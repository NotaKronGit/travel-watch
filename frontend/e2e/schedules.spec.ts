...
test('timetable tab preserves station time, shows unknown transfers and stops polling after completion',async({page})=>{
 ...
 await page.route('**/travelwatch.cabinet.v1.TripService/GetTripRoutes',r=>{
  ...
  calls++;
  return r.fulfill({json:{result:{sources:[],scheduleCheck:ready?{state:'done',incomplete:true,checkedAt:'2026-09-20T10:00:00Z',requests:12,warnings:['Тестовые расписания'],schemes:[{schemeNumber:1,accessVariant:2,state:'unverified',warnings:['Неизвестно время переезда: Вокзал → Аэропорт'],journeys:[{timingVerified:false,legs:[{from:'Курск',to:'Москва',mode:'train',number:'ТЕСТ-123',departure:'2027-01-10T23:00:00+03:00',arrival:'2027-01-11T06:00:00+03:00'}]}]}]}:{state:'running'}}}});
 });
 ...
 await expect(page.getByText('Не удалось подтвердить стыковку',{exact:true})).toBeVisible({timeout:10000});
 await expect(page.getByText('Схема 1 · Подвоз 2')).toBeVisible();
 await expect(page.getByText('Отправление:',{exact:false})).toContainText('23:00');
 await expect(page.getByText('Отправление:',{exact:false})).toContainText('+03:00');
 await expect(page.getByText('Неизвестно время переезда:',{exact:false})).toBeVisible();
