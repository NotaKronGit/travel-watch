...
test('saved schemes paginate independently, survive errors and update only on progress',async({page})=>{
 ...
 expect(copied).toContain('Экспорт содержит все сохранённые варианты каждого источника.');
 expect(copied).toContain('Варианты 1–7 из 7');
 expect(copied).toContain('Курск → Москва, вариант 7');
 expect(copied).not.toContain('Вариант 8');
 const downloadPromise=page.waitForEvent('download');
 await page.getByRole('button',{name:'Скачать все маршруты TXT',exact:true}).click();
