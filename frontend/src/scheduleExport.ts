...
export function scheduleToJson(result: ScheduleCheck, by: JourneySort = 'wait', filter: JourneyFilter = {}): string {
  ...
  return JSON.stringify({
    ...
    nextCheckAt: result.nextCheckAt ? new Date(Number(result.nextCheckAt.seconds) * 1000).toISOString() : null,
    refreshFailedAt: result.refreshFailedAt ? new Date(Number(result.refreshFailedAt.seconds) * 1000).toISOString() : null,
